<template>
  <div class="heat-map-container">
    ...
    <div class="ticker-section">
      <p class="counter-label">Wayground is widely adopted and loved in texas ❤️</p>
      ...
      <p class="counter-sublabel">Questions answered in the state of texas</p>
    </div>

    <div class="map-wrapper">
      <!-- Optional: clean Texas map image with light pink tint (add public/texas-map.png to use) -->
      <div v-show="texasMapImageLoaded" class="texas-map-image-wrap">
        <img
          src="/texas-map.png"
          alt="Texas"
          class="texas-map-img"
          @load="texasMapImageLoaded = true"
          @error="texasMapImageLoaded = false"
        >
      </div>
      <svg
        viewBox="0 0 900 700"
        class="region-map"
        :class="{ 'region-map--behind-image': texasMapImageLoaded }"
      >
        <!-- Background -->
...
// Current Location (Oakland Middle School)
...
const texasMapImageLoaded = ref(false);
...
/* Counter Ticker - scales to fit container so it never overflows */
.ticker-section {
  display: flex;
...
/* Scale counter to fit when container is narrow (e.g. right sidebar) */
@container heatmap (max-width: 520px) {
  .ticker-section .counter-label {
    font-size: 1.5rem;
  }
  .counter-slot {
    width: 34px;
...
@container heatmap (max-width: 420px) {
  .ticker-section .counter-label {
    font-size: 1.35rem;
  }
  .counter-slot {
...
@container heatmap (max-width: 320px) {
  .ticker-section .counter-label {
    font-size: 1.1rem;
  }
  .counter-slot {
...
/* Clean Texas map image (when public/texas-map.png is present) + light pink tint */
.texas-map-image-wrap {
  position: absolute;
  inset: 0;
  z-index: 1;
  border-radius: 12px;
